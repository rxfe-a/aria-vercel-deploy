...
function LoadCustom() {
    const fart = localStorage.getItem("custom-balls");
  
    if (fart) {
        const jsonContainer = document.getElementById('HOLDER_BS')
      const itemJSON = JSON.parse(localStorage.getItem("custom-balls"));
...
function countcustom() {
  ...
  function Favorite(title) {
    if (typeof(Storage) !== "undefined") {
        let favorites = localStorage.getItem("favorites-app");
        if (favorites) {
            favorites += "\n" + title;
        } else {
            favorites = title;
        }
        localStorage.setItem("favorites-app", favorites);
        console.log(title);
    } else {
...
function LoadFavorites() {
    ...
    if (!favorites) {
        console.log("No favorites found.");
        return; // No need to continue if there are no favorites
    }
